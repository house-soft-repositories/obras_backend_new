...
describe('OrgaoRepository', () => {
  ...
  it('persists organizations in the verified schema and lists them by name', async () => {
    ...
    const result = await context.run(tenantContext(tenancy), async () => {
      ...
      const zeta = OrgaoEntity.create(
        orgaoProps(localidade.getOrThrow().id, 'Zeta'),
      );
    ...
    expect(page.pageData.map((item) => item.nome)).toEqual(['Alpha', 'Zeta']);
