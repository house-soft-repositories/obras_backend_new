...
export default class FonteRepository implements IFonteRepository {
  ...
  async findByCodigo(
    ...
    try {
      ...
      const [row] = await this.dataSource.query<FonteModel[]>(
        `SELECT id, nome, descricao, codigo, tipo, valor_previsto AS "valorPrevisto", vigencia, ativo, created_at AS "createdAt", updated_at AS "updatedAt" FROM "${schema}"."fontes" WHERE codigo=$1`,
        [codigo],
      );
  ...
  async findById(id: string): AsyncResult<AppException, FonteEntity | null> {
    try {
      const schema = this.tenantContext.require().schemaName;
      const [row] = await this.dataSource.query<FonteModel[]>(
        `SELECT id, nome, descricao, codigo, tipo, valor_previsto AS "valorPrevisto", vigencia, ativo, created_at AS "createdAt", updated_at AS "updatedAt" FROM "${schema}"."fontes" WHERE id=$1`,
        [id],
      );
      return right(row ? FonteMapper.toEntity(row as any) : null);
    } catch (cause) {
  ...
  async findAll(
    ...
    try {
      ...
      const rows = await this.dataSource.query<FonteModel[]>(
        `SELECT id, nome, descricao, codigo, tipo, valor_previsto AS "valorPrevisto", vigencia, ativo, created_at AS "createdAt", updated_at AS "updatedAt" FROM "${schema}"."fontes" ORDER BY nome ${pageOptions.order} LIMIT $1 OFFSET $2`,
      ...
      return right(
        new PageEntity(
          rows.map((r) => FonteMapper.toEntity(r as any)),
          meta,
        ),
      );
    } catch (cause) {
